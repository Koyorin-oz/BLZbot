...
async function calculateTotalPossiblePoints(guild) {
  ...
  if (!staffRole) {
    console.log(
      `[Vote] ⚠️ Rôle Staff (${STAFF_ROLE_ID}) non trouvé dans le cache`,
    );
    return 0;
  }
 ...
 */
async function endVoteProgrammatically(
  message,
  guild,
  voteManager,
  forceVoteKey = null,
  ...
  if (staffProfileDb && !promoteUser) {
    if (vote.type === "candidature") {
      ...
      staffProfileDb.run(
        ...
        (err) => {
          if (err) console.error("Erreur MAJ candidature refusée:", err);
        },
      );
      ...
      staffProfileDb.run(
        ...
        async (err) => {
          if (err) {
            ...
            if (member) {
              await member.roles
                .remove(CONFIG.MODO_TEST_ROLE_ID)
                .catch(() => null);
              const serverName = guild?.name || "BLZstarss";
              ...
              const failureEmbed = new EmbedBuilder()
                ...
                .setDescription(
                  "Votre période de test s'arrête malheureusement ici, mais votre engagement nous a énormément apporté ! Merci à vous de nous avoir aidé !\n\n**Nous espérons vous revoir bientôt !**",
                )
                .setThumbnail(
                ...
                .setFooter({ text: "BLZstarss • L'Equipe de modération" })
                .setTimestamp()
                .setColor("#FFA500");
      ...
      try {
        switch (vote.type) {
          case "candidature":
            ...
            if (staffProfileDb) {
              staffProfileDb.run(
                ...
                ["vote_system", Date.now(), voteKey],
                (err) => {
                  if (err) console.error("Erreur MAJ candidature:", err);
                },
              );
              ...
              staffProfileDb.run(
                ...
                (err) => {
                  if (err) console.error("Erreur modo test period:", err);
                },
              );
              ...
              staffProfileDb.run(
                ...
                [
                  voteKey,
                  CONFIG.MODO_TEST_ROLE_ID,
                  "Modérateur Test",
                  startTimestamp,
                  "vote_system",
                ],
                (err) => {
                  if (err) console.error("Erreur promotion:", err);
                },
              );
              ...
              staffProfileDb.run(
                ...
                (err) => {
                  if (err)
                    console.error("Erreur déduction chance modo test:", err);
                },
              );
            }
            break;
          ...
          case "modo_test_to_modo":
            ...
            if (staffProfileDb2) {
              staffProfileDb2.run(
                ...
                ["vote_system", voteKey],
                (err) => {
                  if (err) console.error("Erreur fin modo test:", err);
                },
              );
              ...
              staffProfileDb2.run(
                ...
                (err) => {
                  if (err) console.error("Erreur promotion modo:", err);
                },
              );
            }
            break;
          ...
          case "superviseur_to_admin_test":
            ...
            const promoEmbed2 = new EmbedBuilder()
          ...
          case "admin_test_to_admin":
            await member.roles
              .remove(CONFIG.ADMIN_TEST_ROLE_ID)
              .catch(() => null);
            await member.roles.add(CONFIG.ADMIN_ROLE_ID).catch(() => null);
            const promoEmbed3 = new EmbedBuilder()
...
module.exports = {
  ...
  async execute(
    interaction,
    { voteManager, recruitmentManager, config, client },
  ) {
...
async function handleStandardVote(interaction, voteManager, customId) {
  ...
  if (!embed) {
    return interaction.reply({
      content: "Erreur: Embed introuvable.",
      ephemeral: true,
    });
  }
  ...
  if (memberPoints === 0) {
    return interaction.reply({
      content: "Vous n'avez pas les permissions pour voter.",
      ephemeral: true,
    });
  }
  ...
  if (previousVote === voteType) {
    return interaction.reply({
      content: `Vous avez déjà voté ${voteType}.`,
      ephemeral: true,
    });
  }
  ...
  await interaction.reply({
    content: `Vote enregistré: ${voteType}`,
    ephemeral: true,
  });
  ...
  const totalPossiblePoints = await calculateTotalPossiblePoints(
    interaction.guild,
  );
  ...
  if (vote.oui >= majorityNeeded || vote.non >= majorityNeeded) {
    ...
    console.log(
      `🎯 Majorité absolue atteinte: ${winner} (${winnerPoints}/${totalPossiblePoints} points, majorité à ${majorityNeeded})`,
    );

    setTimeout(async () => {
      try {
        await handleEndVote(
          { message, guild: interaction.guild, reply: () => {} },
          voteManager,
        );
      } catch (error) {
...
async function handleDebanVote(interaction, voteManager, customId, client) {
  ...
  if (!voteManager.debanVotes[targetUserId]) {
    return interaction.reply({
      content: "Vote de débannissement introuvable.",
      ephemeral: true,
    });
  }
  ...
  if (interaction.user.id === targetUserId) {
    return interaction.reply({
      content:
        "❌ Vous ne pouvez pas voter sur votre propre demande de débannissement.",
      ephemeral: true,
    });
  }
  ...
  await message.edit({ embeds: [updatedEmbed] });
  await interaction.reply({
    content: `Vote enregistré : **${voteType}** (${memberPoints} pts).`,
    ephemeral: true,
  });
  ...
  try {
    ...
    if (
      ...
      (vote.oui >= majorityNeeded || vote.non >= majorityNeeded)
    ) {
      const winner = vote.oui >= majorityNeeded ? "OUI" : "NON";
      console.log(
        `[Deban] Majorité absolue atteinte pour ${targetUserId}: ${winner} (${vote.oui} oui / ${vote.non} non, seuil ${majorityNeeded}/${totalPossiblePoints})`,
      );
      setTimeout(async () => {
        try {
          await endDebanVoteProgrammatically(
            message,
            interaction.guild,
            voteManager,
            client,
 ...
 */
async function endDebanVoteProgrammatically(
  message,
  guild,
  voteManager,
  client,
  ...
  if (accepted) {
    try {
      ...
      await mainGuild.bans.remove(
        targetUserId,
        "Débannissement accepté par vote",
      );
    } catch (error) {
      if (error?.code !== 10026) {
        // 10026 = Unknown Ban (déjà débanni)
  ...
  try {
    ...
    if (user) {
      const dmEmbed = new EmbedBuilder()
        ...
        .setTitle(
          accepted
            ? "✅ Demande de débannissement acceptée"
            : "❌ Demande de débannissement refusée",
        )
        .setDescription(
          accepted
  ...
  if (message.channel?.isTextBased?.()) {
    if (accepted) {
      ...
      await message.channel
        .send(
          `❌ La demande de débannissement de <@${targetUserId}> a été refusée.`,
        )
        .catch(() => null);
    }
  }
  ...
  if (vote.forumMode && message.channel?.isThread?.() && resultEmbed && embed) {
    ...
    if (testGuildId) {
      ...
      console.warn(
        "[Deban] forumMode sans mapping testGuildId — fallback édition message seule.",
      );
      const disabledRow = message.components[0];
...
async function handleEndVote(interaction, voteManager) {
  ...
  if (!result.success) {
    return interaction.reply({
      content: `Erreur: ${result.result}`,
      ephemeral: true,
    });
  }

  await interaction.reply({
    content: `Le vote a été terminé. Résultat: ${result.result}`,
    ephemeral: true,
  });
}
...
async function handleEndDebanVote(interaction, voteManager, client, config) {
  ...
  if (!res.success) {
    return interaction.editReply({
      content: `⚠️ Impossible de terminer le vote (${res.reason ?? "erreur inconnue"}).`,
    });
  }
  await interaction.editReply({
    content: `✅ Vote terminé. Résultat : **${res.result}**.`,
  });
}
...
async function handleAcceptReglement(interaction) {
  ...
  if (!roleId || roleId === "ID_DU_ROLE_REGLEMENT_ACCEPTE") {
    return interaction.editReply({
      content:
        '⚠️ Le rôle "Règlement accepté" n\'est pas configuré dans config.js',
    });
  }
  ...
  try {
    ...
    await interaction.editReply({
      content:
        "✅ Vous avez accepté le règlement ! Le rôle vous a été attribué.",
    });
  } catch (error) {
...
async function handleFinishReglement(interaction) {
  ...
  try {
    ...
    await reglementCommand.sendOrUpdateReglement(
      interaction,
      reglementNom,
      false,
    );
...
async function handleRecruitmentVote(
  ...
  if (!voteManager.votes[userId]) {
    ...
    console.log(
      `[Candidature] Votes disponibles: ${Object.keys(voteManager.votes).join(", ") || "aucun"}`,
    );
  ...
  if (voterRolePoints < minPointsToVote) {
    const permEmbed = new EmbedBuilder()
      ...
      .setDescription(
        "Vous devez être au minimum **Modérateur** pour voter sur une candidature.",
      )
      .setColor("#FF0000")
      .setTimestamp();
  ...
  if (actionType === "oui" || actionType === "non") {
    ...
    if (
      recruitmentVote[oppositeVote] &&
      recruitmentVote[oppositeVote][voterId]
    ) {
    ...
    const totalNon = Object.values(recruitmentVote.non || {}).reduce(
      (a, b) => a + b,
      0,
    );
    const totalPossiblePoints = await calculateTotalPossiblePoints(
    ...
    const ouiPercentage =
      totalPossiblePoints > 0
        ? Math.min(100, Math.round((totalOui / totalPossiblePoints) * 100))
        : 0;
    const nonPercentage =
      totalPossiblePoints > 0
        ? Math.min(100, Math.round((totalNon / totalPossiblePoints) * 100))
        : 0;
  ...
  // Gestion de la fin du vote
    ...
    const adminRole = CONFIG.STAFF_ROLES.find(
    ...
    if (voterRolePoints < minPointsToEnd) {
      const adminEmbed = new EmbedBuilder()
        .setTitle("🔒 Permissions Insuffisantes")
        .setDescription(
          "Seuls les **Administrateurs** peuvent terminer un vote de recrutement.",
        )
        .setColor("#FF0000")
        .setTimestamp();
    ...
    if (totalOui > totalNon) {
      try {
        ...
        if (staffProfileDb) {
          staffProfileDb.run(
            ...
            (err) => {
              if (err)
                console.error(
                  "Erreur MAJ candidature acceptée (recrutement):",
                  err,
                );
            },
          );
          ...
          staffProfileDb.run(
            ...
            (err) => {
              if (err)
                console.error(
                  "Erreur création modo test period (recrutement):",
                  err,
                );
            },
          );
          ...
          staffProfileDb.run(
            ...
            (err) => {
              if (err)
                console.error(
                  "Erreur insertion promotion modo test (recrutement):",
                  err,
                );
            },
          );
          ...
          staffProfileDb.run(
            ...
            (err) => {
              if (err)
                console.error(
                  "Erreur déduction chance modo test (recrutement):",
                  err,
                );
            },
          );
        ...
        await candidate
          ...
          .catch((err) =>
            console.error("Impossible d'envoyer le MP au candidat:", err),
          );
        ...
        const acceptEmbed = new EmbedBuilder()
          .setTitle("✅ Candidature Acceptée")
          .setDescription(
            `La candidature de **${candidate.tag}** a été **acceptée** avec succès !`,
          )
          .addFields(
            { name: "👤 Candidat", value: `${candidate.tag}`, inline: true },
            {
              name: "📊 Résultat",
              value: `**${totalOui}** OUI vs **${totalNon}** NON`,
              inline: true,
            },
            {
              name: "🎭 Rôles attribués",
              value: `<@&${CONFIG.MODO_TEST_ROLE_ID}>`,
              inline: false,
            },
          )
          .setColor("#00FF00")
        ...
        const errorEmbed = new EmbedBuilder()
          .setTitle("❌ Erreur")
          .setDescription(
            "Une erreur est survenue lors de l'attribution des rôles.",
          )
          .setColor("#FF0000")
          .setTimestamp();
    ...
    // Si le vote est refusé
      ...
      if (staffProfileDb) {
        staffProfileDb.run(
          ...
          (err) => {
            if (err)
              console.error(
                "Erreur MAJ candidature refusée (recrutement):",
                err,
              );
          },
        );
      ...
      const refuseEmbed = new EmbedBuilder()
        .setTitle("❌ Candidature Refusée")
        .setDescription(
          `La candidature de **${candidate.tag}** a été **refusée**.`,
        )
        .addFields(
          { name: "👤 Candidat", value: `${candidate.tag}`, inline: true },
          {
    ...
    // Égalité
      ...
      const equalityEmbed = new EmbedBuilder()
        .setTitle("⚖️ Égalité")
        .setDescription(
          "Le vote est à égalité. Veuillez continuer à voter avant de terminer.",
        )
        .setColor("#FFA500")
        .setTimestamp();
