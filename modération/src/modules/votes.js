...
function getPromotionLabel(promotionType) {
  const promo = CONFIG.PROMOTION_PATHS.find(
    (path) => path.value === promotionType,
  );
  if (promo && promo.name) return promo.name;
...
class VoteManager {
  ...
  hasActiveDebanRequest(userId) {
    if (this.debanVotes?.[userId])
      return { active: true, reason: "vote", data: this.debanVotes[userId] };
    if (this.pendingDebanRequests?.[userId])
      return {
        active: true,
        reason: "pending",
        data: this.pendingDebanRequests[userId],
      };
    const cd = this.debanCooldowns?.[userId];
    if (cd && Number(cd.until) > Date.now())
      return { active: true, reason: "cooldown", data: cd };
    if (this.activeDebanRequests.has(userId))
      return { active: true, reason: "in_progress", data: null };
  ...
  setFormData(userId, data, ttlMs = FORM_DATA_TTL_MS) {
    ...
    if (this.formDataTimers.has(userId))
      clearTimeout(this.formDataTimers.get(userId));
  ...
  loadVotes() {
    ...
    if (!_V_COMPACT)
      console.log(`[VoteManager] Chargement votes: ${votesFilePath}`);
    ...
    if (fs.existsSync(votesFilePath)) {
      try {
        ...
        if (!_V_COMPACT)
          console.log(`[VoteManager] ${voteCount} vote(s) chargé(s)`);
        return votes;
      } catch (e) {
  ...
  loadDebanVotes() {
    ...
    if (fs.existsSync(debanVotesPath)) {
      try {
        ...
        console.error(
          "Erreur lors du chargement des votes de débannissement:",
          e,
        );
        return {};
      }
  ...
  saveDebanVotes() {
    ...
    fs.writeFileSync(
      debanVotesPath,
      JSON.stringify(this.debanVotes, null, 2),
      "utf8",
    );
  }
   ...
   */
  loadCandidateVotes() {
    const candidateVotesPath = path.join(
      __dirname,
      "../../candidate_votes.json",
    );
  ...
  saveCandidateVotes() {
    ...
    fs.writeFileSync(
      candidateVotesPath,
      JSON.stringify(this.candidateVotes, null, 2),
      "utf8",
    );
  }
  ...
  savePendingDebanRequests() {
    ...
    fs.writeFileSync(
      pendingPath,
      JSON.stringify(this.pendingDebanRequests, null, 2),
      "utf8",
    );
  }
  ...
  saveDebanCooldowns() {
    ...
    fs.writeFileSync(
      p,
      JSON.stringify(this.debanCooldowns || {}, null, 2),
      "utf8",
    );
  }
  ...
  async startRankupVote(user, promotionType, interaction) {
    ...
    const promotionLabel = getPromotionLabel(promotionType);
    const embed = new EmbedBuilder()
  ...
  async startDebanVote(
    ...
    if (!targetChannel || (!isForumChannel && !targetChannel.isTextBased?.())) {
      console.error(
        `[Deban] Salon de vote introuvable ou type non supporté (${channelId}).`,
      );
      await interaction.followUp({
        content:
          "❌ Votre demande a été reçue, mais le vote n'a pas pu être lancé (salon de vote introuvable). Contactez un administrateur.",
        ephemeral: true,
      });
    ...
    if (isForumChannel) {
      ...
      try {
        ...
        if (!sentMessage) {
          throw new Error(
            "Starter message introuvable après création du post forum.",
          );
        }
    ...
    await interaction.followUp({
      content:
        "✅ Votre demande de débannissement a été envoyée et un vote a été lancé. Vous serez averti en privé dès que le résultat tombera.",
      ephemeral: true,
    });
  ...
  async processPendingDebanRequests(client) {
    if (!client) return 0;
    const now = Date.now();
    const eligibleIds = Object.keys(this.pendingDebanRequests || {}).filter(
      (uid) => {
    ...
    for (const uid of eligibleIds) {
      ...
      try {
        ...
        const mentionRoleId =
          req.mentionRoleId ||
          CONFIG.STAFF_ROLES.find((r) => r.name === "Staff")?.id ||
          "1172237685763608579";
        ...
        if (
          !targetChannel ||
          (!isForumChannel && !targetChannel.isTextBased?.())
        ) {
          console.error(
            `[Deban] processPending : salon ${channelId} introuvable ou type non supporté, on conserve la demande.`,
          );
          continue;
        }
        ...
        if (this.debanVotes[uid]) {
          console.warn(
            `[Deban] processPending : un vote existe déjà pour ${uid}, suppression de la pending.`,
          );
        ...
        if (isForumChannel || req.forumMode) {
          ...
          if (!testGuildId) {
            ...
            this.saveDebanVotes();
            console.error(
              `[Deban] processPending : forum ${channelId} sans entrée deban_forum_config.json, skip ${uid}.`,
            );
            continue;
          }
          ...
          try {
            ...
            this.saveDebanVotes();
            console.error(
              `[Deban] processPending : échec forum pour ${uid}:`,
              e?.message || e,
            );
            continue;
          }
          ...
          const { embed, row } = this._buildDebanVoteComponents(
            ...
            {
              name: "⏳ Statut",
              value: "Mise en attente expirée — vote automatique lancé.",
              inline: false,
            },
          );
        ...
        try {
          ...
          /* user introuvable ou DM fermés */
        }

        processed++;
      } catch (err) {
        console.error(
          `[Deban] processPending : erreur sur ${uid}:`,
          err?.message || err,
        );
      }
    }
