...
function updateStreak(client, userId) {
  try {
    const getUserStmt = db.prepare(
      "SELECT streak, last_streak_timestamp FROM users WHERE id = ?",
    );
    const user = getUserStmt.get(userId);

    if (!user) {
      logger.warn(
        `Utilisateur ${userId} non trouvé pour mettre à jour la streak`,
      );
    ...
    logger.error(
      `Erreur lors de la mise à jour de la streak pour ${userId}:`,
      error,
    );
    return { streakUpdated: false, newStreak: 0 };
  }
  ...
  try {
    ...
    if (!streakChannelId) {
      if (!_warnedStreakChannel) {
        ...
        logger.warn(
          "STREAK_CHANNEL (ou TEST_STREAK_CHANNEL sur le serveur test) non défini — annonce streak ignorée.",
        );
      }
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Bravo <@${userId}> qui a maintenant une streak de **${newStreak} jours** !`,
      )
      .setColor(0xffa500);
    ...
    if (reward.stars > 0) {
      embed.addFields({
        name: "Récompense",
        value: `Tu viens de gagner **${reward.stars.toLocaleString()} starss**`,
      });
    } else if (reward.item) {
      const itemName =
        reward.item === "coffre_normal" ? "1 Coffre Bonus" : reward.item;
      embed.addFields({
        name: "Récompense",
        value: `Tu viens de gagner **${itemName}**`,
      });
    }
    ...
    logger.info(
      `Annonce de streak envoyée pour ${userId} (${newStreak} jours)`,
    );
  } catch (error) {
  ...
  schedule.scheduleJob(rule, () => {
    try {
      ...
      const resetStmt = db.prepare(
        "UPDATE users SET streak = 0 WHERE last_streak_timestamp < ? AND streak > 0",
      );
      const result = resetStmt.run(yesterdayTimestamp);

      if (result.changes > 0) {
        logger.info(
          `[STREAK] Reset automatique de minuit : ${result.changes} utilisateur(s) ont perdu leur streak (pas d'activité hier).`,
        );
      }
    } catch (error) {
      logger.error(
        "[STREAK] Erreur lors du reset automatique des streaks:",
        error,
      );
    }
  });
