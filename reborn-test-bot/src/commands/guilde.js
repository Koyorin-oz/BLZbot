...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('guilde')
    .setDescription(d('🛡️', 'Guildes joueur — création, membres, trésorerie et focus.'))
    .addSubcommand((sc) =>
      sc
        .setName('creer')
    ...
    .addSubcommand((sc) =>
      sc
        .setName('inviter')
        .setDescription('Inviter un membre — il reçoit une invitation à accepter (chef ou permission « invitations »)')
  ...
  async execute(interaction) {
    ...
    if (sub === 'dissoudre') {
      ...
      if (!r.ok) return interaction.reply({ content: r.error });
      return interaction.reply({ content: 'Guilde dissoute. Toutes ses données ont été effacées.' });
    }
