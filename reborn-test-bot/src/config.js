...
/** Rôle Discord « Hackeur » (jeton hacker / panneau /salon-hacker). */
const hackerRoleId =
  (process.env.REBORN_HACKER_ROLE_ID || process.env.HACKER_ROLE_ID || '1432469784653467701').trim() || null;
...
/**
 * 1 (défaut) = enregistrer aussi les slash du module **niveau** (`/profil`, `/daily`, …) sur cette application.
 * 0 = uniquement `reborn-test-bot/src/commands/` → **pas de `/profil`** tant que tu n’ajoutes pas une commande locale du même nom.
 */
const mirrorNiveauSlash =
  String(process.env.REBORN_MIRROR_NIVEAU_SLASH || '1').trim() !== '0';
