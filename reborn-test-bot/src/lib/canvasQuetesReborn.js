...
async function loadSafe(urlOrPath) {
  ...
  try {
    ...
    if (fs.existsSync(urlOrPath))
      return await loadImage(fs.readFileSync(urlOrPath));
    return null;
  } catch {
    return null;
  }
...
function drawHeader(ctx, displayName, avatarImg) {
  ...
  ctx.fillText(
    "Récompenses automatiques dès que le seuil est atteint.",
    tx,
    ty + 30,
  );
...
function drawCard(ctx, x, y, w, h, theme, data) {
  ...
  // Récompense : si badge de status présent, l'afficher juste en-dessous, sinon en bas droite
  if (data.rewardText) {
    ctx.font = "700 20px InterBold, Arial";
    ctx.fillStyle = TITLE_COLOR;
    if (data.statusText) {
      ctx.textAlign = "center";
      // placer sous le badge avec un petit padding
      ctx.fillText(data.rewardText, bx + bw / 2, by + bh + 22);
      ctx.textAlign = "left";
    } else {
      ctx.textAlign = "right";
...
async function renderQuetesRebornPng(opts) {
  ...
  // Selection
  {
    ...
    if (!sid) {
      ...
      subtitle =
        "Récompense : auto si « messages », sinon clique sur « Réclamer ».";
      statusText = "EN COURS";
    }
  ...
  let spawnerText;
  if (spawner.locked)
    spawnerText = "🔒 Event Spawner hebdo : palier 5 Événement.";
  else if (spawner.available)
    spawnerText = "🎁 Event Spawner hebdo disponible !";
  else {
